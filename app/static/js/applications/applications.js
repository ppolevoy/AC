...
(function() {
    ...
    const ModalManager = {
        ...
        /**
         * Проверяет, выбраны ли все экземпляры группы
         * @param {Array} selectedApps - выбранные приложения
         * @returns {boolean}
         */
        _isAllGroupInstancesSelected(selectedApps) {
            if (!selectedApps || selectedApps.length === 0) return false;

            const groupName = selectedApps[0].group_name || selectedApps[0].name;
            const allAppsInGroup = StateManager.state.allApplications.filter(app =>
                (app.group_name || app.name) === groupName
            );

            return selectedApps.length === allAppsInGroup.length;
        },
        ...
        async showSimpleUpdateModal(apps, title) {
            ...
            // Проверяем условия для режима "В рестарт"
            const hasDockerApps = apps.some(app => app.type === 'docker');
            const allGroupSelected = this._isAllGroupInstancesSelected(apps);
            const disableNightRestart = hasDockerApps || !allGroupSelected;
            const nightRestartTitle = hasDockerApps
                ? 'Недоступно для Docker-приложений'
                : (!allGroupSelected ? 'Доступно при выборе всех экземпляров в группе' : '');
                ...
                <form id="update-form" class="modal-form">
                    ...
                    <div class="form-group">
                        ...
                        <div class="radio-group">
                            ...
                            <label class="radio-label ${disableNightRestart ? 'disabled' : ''}" ${nightRestartTitle ? `title="${nightRestartTitle}"` : ''}>
                                <input type="radio" name="mode" value="night-restart" ${disableNightRestart ? 'disabled' : ''}> В рестарт
                            </label>
                        </div>
                    </div>
                ...
                formHTML += `
                    <div class="form-group animated-fade-in" style="animation-delay: 0.3s">
                        ...
                        <div class="radio-group">
                            ...
                            <label class="radio-label ${disableNightRestart ? 'disabled' : ''}" ${nightRestartTitle ? `title="${nightRestartTitle}"` : ''}>
                                <input type="radio" name="mode" value="night-restart" ${state.restartMode === 'night-restart' && !disableNightRestart ? 'checked' : ''} ${disableNightRestart ? 'disabled' : ''}> В рестарт
                            </label>
                        </div>
                    </div>
    ...
    const AutoRefresh = {
        ...
        start() {
            ...
            if (this.isEnabled) {
                this._log(`🔄 Автообновление запущено (каждые ${this.intervalSeconds}с)`);

                this.intervalId = setInterval(async () => {
                    // Page Visibility API - не обновляем неактивные вкладки
                    if (document.hidden) {
                        this._log('👁️ Пропуск обновления: вкладка неактивна');
                        return;
                    }

                    // Race condition - проверяем, не идет ли уже загрузка
                    if (this.isLoading) {
                        this._log('⏭️ Пропуск обновления: предыдущее обновление еще выполняется', 'warn');
                        return;
                    }

                    // getComputedStyle вместо inline styles
                    ...
                    if (isModalOpen && this.skippedUpdates < this.maxSkippedUpdates) {
                        this.skippedUpdates++;
                        this._log(`⏸️ Пропуск обновления ${this.skippedUpdates}/${this.maxSkippedUpdates}: открыто модальное окно`);
                        return;
                    }

                    // Принудительное обновление если достигнут лимит пропусков
                    if (this.skippedUpdates >= this.maxSkippedUpdates) {
                        this._log('⚠️ Принудительное обновление: превышен лимит пропусков', 'warn');
                    }

                    // Сбрасываем счетчик
                    this.skippedUpdates = 0;

                    // Обработка ошибок + флаг isLoading
                    try {
                        this.isLoading = true;
                        this._log('🔄 Автоматическое обновление данных...');
                        await EventHandlers.loadApplications();
                    } catch (error) {
                        // Ошибки всегда показываем, независимо от debug режима
                ...
                document.addEventListener('visibilitychange', () => {
                    if (!document.hidden && this.isEnabled && !this.isLoading) {
                        this._log('👁️ Вкладка стала активной, запускаем обновление');
                        this.restart();
                    }
                });
            }
        ...
        stop() {
            if (this.intervalId) {
                ...
                this.skippedUpdates = 0;
                this._log('⏸️ Автообновление остановлено');
            }
        },

        restart() {
            this._log('🔄 Перезапуск таймера автообновления');
            this.skippedUpdates = 0;
            this.stop();
            this.start();
        },
