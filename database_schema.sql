...
-- ============================================================================
-- Скрипт для создания полной схемы базы данных PostgreSQL
-- Включает все таблицы, индексы, внешние ключи и ограничения
-- ============================================================================

-- ============================================================================
-- ОЧИСТКА БАЗЫ ДАННЫХ (DROP TABLES)
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- Серверы (физические или виртуальные хосты)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Справочник приложений (базовая информация о типах приложений)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Группы приложений (для группового управления и настроек)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Теги (для маркировки приложений и групп)
-- ----------------------------------------------------------------------------
CREATE TABLE tags (
    id SERIAL PRIMARY KEY,
    name VARCHAR(64) UNIQUE NOT NULL,
...
-- ----------------------------------------------------------------------------
-- Экземпляры приложений (реальные инстансы на серверах)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Связь приложений с тегами (many-to-many)
-- ----------------------------------------------------------------------------
CREATE TABLE application_instance_tags (
    id SERIAL PRIMARY KEY,
    application_id INTEGER REFERENCES application_instances(id) ON DELETE CASCADE NOT NULL,
...
-- ----------------------------------------------------------------------------
-- Связь групп с тегами (many-to-many)
-- ----------------------------------------------------------------------------
CREATE TABLE application_group_tags (
    id SERIAL PRIMARY KEY,
    group_id INTEGER REFERENCES application_groups(id) ON DELETE CASCADE NOT NULL,
    tag_id INTEGER REFERENCES tags(id) ON DELETE CASCADE NOT NULL,
    assigned_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    assigned_by VARCHAR(64),
...
-- ----------------------------------------------------------------------------
-- События (история действий с приложениями)
-- ----------------------------------------------------------------------------
CREATE TABLE events (
    id SERIAL PRIMARY KEY,
...
-- ----------------------------------------------------------------------------
-- HAProxy серверы (члены backend'ов)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- История маппинга HAProxy серверов на приложения
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Унифицированная таблица маппингов приложений
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Задачи (для выполнения операций)
-- ----------------------------------------------------------------------------
CREATE TABLE tasks (
...
CREATE INDEX idx_task_server ON tasks(server_id);
...
CREATE TABLE application_version_history (
    id SERIAL PRIMARY KEY,
    instance_id INTEGER REFERENCES application_instances(id) ON DELETE CASCADE NOT NULL,
...
COMMENT ON TABLE application_catalog IS 'Справочник типов приложений (без привязки к серверам)';
COMMENT ON TABLE application_groups IS 'Группы приложений для управления и настроек';
COMMENT ON TABLE application_instances IS 'Экземпляры приложений на конкретных серверах';
COMMENT ON TABLE tags IS 'Теги для маркировки приложений и групп';
COMMENT ON TABLE events IS 'История событий и действий';
COMMENT ON TABLE haproxy_instances IS 'HAProxy инстансы на серверах';
COMMENT ON TABLE haproxy_backends IS 'HAProxy backend пулы';
COMMENT ON TABLE haproxy_servers IS 'Серверы в HAProxy backend пулах';
COMMENT ON TABLE eureka_servers IS 'Eureka registry серверы';
COMMENT ON TABLE eureka_applications IS 'Приложения в Eureka';
...
COMMENT ON TABLE tasks IS 'Очередь задач для выполнения операций';
COMMENT ON TABLE orchestrator_playbooks IS 'Метаданные orchestrator playbooks';
COMMENT ON TABLE mailing_groups IS 'Группы email-рассылки для уведомлений';
...
-- Создание последовательностей завершено автоматически через SERIAL
